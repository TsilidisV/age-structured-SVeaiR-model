...
function contacts_figure()
    ...
    c_2(x) = (16.71/0.37)*exp(-((x-(360.0*10)) / 10000 )^2)
    c_1(x) = (16.71/0.38)*exp(-((x-(360*80)) / 10000 )^2)
...
with_theme(contacts_figure, total_theme)
save("figures/figure2.pdf", with_theme(contacts_figure, total_theme), pt_per_unit = 1)
...
function asymptomatics_figure()
    ...
    ax = Axis(fig[1,1], xlabel = L"$\theta \; (\cdot 360$ days)", 
        xticks = (0:(360*10):360*90, powerOfTen.(0:10:90)),
        ylabel = L"q(\theta) \; (%)",
        title = L"\text{\textbf{Percentage of exposed individuals becoming asymptomatic infectious}}",
...
with_theme(asymptomatics_figure, total_theme)
save("figures/figure3.pdf", with_theme(asymptomatics_figure, total_theme), pt_per_unit = 1)
...
save("figures/figure4.pdf", with_theme(figure_R0_leq1, total_theme), pt_per_unit = 1)
...
function figure_R0_more1()
    ...
    axS = Axis(fig[1,1], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", yticks = ([0, 5*10^6, 1*10^7, 1.5*10^7, 2*10^7], [L"0", L"5\cdot10^6", L"1\cdot10^7", L"1.5\cdot10^7", L"2\cdot10^7"]), title = L"\textrm{\textbf{Susceptible}} \; (S)"            )
    axV = Axis(fig[1,2], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", yticks = ([0, 5*10^6, 1*10^7, 1.5*10^7, 2*10^7], [L"0", L"5\cdot10^6", L"1\cdot10^7", L"1.5\cdot10^7", L"2\cdot10^7"]), title = L"\textrm{\textbf{Vaccinated}} \; (V)"              )
    axE = Axis(fig[2,1], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", yticks = ([0, 1*10^7, 2*10^7, 3*10^7], [L"0", L"1\cdot10^7", L"2\cdot10^7", L"3\cdot10^7"]),                            title = L"\textrm{\textbf{Exposed}} \; (E)"                 )
    axA = Axis(fig[2,2], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", yticks = ([0, 5*10^6, 1*10^7], [L"0", L"5\cdot10^6", L"1\cdot10^7"]),                                                   title = L"\textrm{\textbf{Asymptomatic infectious}} \; (A)" )
    axI = Axis(fig[3,1], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", yticks = ([0, 1*10^7, 2*10^7, 3*10^7], [L"0", L"1\cdot10^7", L"2\cdot10^7", L"3\cdot10^7"]),                            title = L"\textrm{\textbf{Symptomatic infectious}} \; (I)" )
    axR = Axis(fig[3,2], xticks = xtickshort, titlesize = 12, xlabel = L"\text{Days}", ytickformat = y -> powerOfTen.(y),                                                                                      title = L"\textrm{\textbf{Removed}} \; (R)"                 )
...
save("figures/figure5.pdf", with_theme(figure_R0_more1, total_theme), pt_per_unit = 1)
