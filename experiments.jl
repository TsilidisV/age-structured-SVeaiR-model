...
# Function that takes the result of the solver and a name and saves the resulting matrix.
# 
...
##### R0 < 1 #####
gC(x) = exp(-((x-(ω*80)) / 10000 )^2)
sum(gC.(0:1:ω*90))/length(0:1:ω*90)

P2 = SVeaiR_Param(N0 = 80.0*10^6, μ = (16*10^(-3))/360, prA = 2/10, prI = 2/5, c = θ -> gC(θ), 
...
##### R0 > 1 #####
gC2(x) = (16.71291/0.38)*exp(-((x-(ω*10)) / 10000 )^2)
sum(gC2.(0:1:ω*90))/length(0:1:ω*90)

P1 = SVeaiR_Param(N0 = 80.0*10^6, μ = (16*10^(-3))/360, prA = 2/10, prI = 2/5, c = θ -> gC2(θ), 
                    p = 10^-3, ε = 0.7, ζ = 1/14, k = θ -> kF(θ), q = θ -> qF(θ), ξ = θ -> 0.5,
                    χ = θ -> χF(θ), γA = θ -> 1/8, γI = θ -> 1/14)
R0(P1)
IC1(i,j) = SVeaiR_ICs(E=j*10.0^i)
